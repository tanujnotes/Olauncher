...
class MainViewModel(application: Application) : AndroidViewModel(application) {
  ...
  val appBlockedMessage = SingleLiveEvent<String>()
  ...
  fun selectedApp(appModel: AppModel, flag: Int) {
    // Check if app is blocked before launching
    if (flag == Constants.FLAG_LAUNCH_APP || flag == Constants.FLAG_HIDDEN_APPS) {
      val appLimit = Prefs(appContext).getAppLimit(appModel.appPackage)
      if (appLimit != null && appLimit.isCurrentlyBlocked()) {
        val message = appContext.getString(R.string.app_blocked_until, appLimit.getEndTimeString())
        appBlockedMessage.postValue(message)
        return
      }
    }
    ...
    when (flag) {
      ...
      Constants.FLAG_SET_APP_LIMIT -> {
        // This flag is handled in AppLimitsFragment
  ...
  fun getTodaysScreenTime() {
    ...
    for ((key, value) in eventsMap) {
      ...
      val totalTimeBackground: Long = backgroundBucketMap.values.mapNotNull { it?.totalTime }.sum()
