...
class Prefs(context: Context) {
  ...
  private val APP_LIMITS = "APP_LIMITS"
  ...
  // App Limits
  fun getAppLimits(): List<AppLimitModel> {
    val limitsJson = prefs.getString(APP_LIMITS, "[]") ?: "[]"
    val limits = mutableListOf<AppLimitModel>()
    try {
      val jsonArray = org.json.JSONArray(limitsJson)
      for (i in 0 until jsonArray.length()) {
        limits.add(AppLimitModel.fromJson(jsonArray.getJSONObject(i)))
      }
    } catch (e: Exception) {
      e.printStackTrace()
    }
  ...
  fun saveAppLimits(limits: List<AppLimitModel>) {
    try {
      val jsonArray = org.json.JSONArray()
      for (limit in limits) {
        jsonArray.put(limit.toJson())
      }
      prefs.edit().putString(APP_LIMITS, jsonArray.toString()).apply()
    } catch (e: Exception) {
      e.printStackTrace()
    }
  }

  fun addAppLimit(limit: AppLimitModel) {
    val limits = getAppLimits().toMutableList()
    // Remove any existing limit for this app
    limits.removeAll { it.appPackage == limit.appPackage }
    limits.add(limit)
    saveAppLimits(limits)
  }

  fun removeAppLimit(appPackage: String) {
    val limits = getAppLimits().toMutableList()
    limits.removeAll { it.appPackage == appPackage }
    saveAppLimits(limits)
  }

  fun getAppLimit(appPackage: String): AppLimitModel? {
    return getAppLimits().find { it.appPackage == appPackage }
  }
}
