...
class AppDrawerFragment : Fragment() {
  ...
  private fun initViews() {
    ...
    else if (flag == Constants.FLAG_SET_APP_LIMIT)
            binding.search.queryHint = getString(R.string.select_app_for_limit)
            ...
            object : SearchView.OnQueryTextListener {
              override fun onQueryTextSubmit(query: String?): Boolean {
                if (query?.startsWith("!") == true)
                        requireContext()
                                .openUrl(Constants.URL_DUCK_SEARCH + query.replace(" ", "%20"))
                else if (adapter.itemCount == 0
                ) // && requireContext().searchOnPlayStore(query?.trim()).not())
    ...
    adapter =
            AppDrawerAdapter(
                    ...
                    appClickListener = {
                      ...
                      // Special handling for app limit selection
                      if (flag == Constants.FLAG_SET_APP_LIMIT) {
                        findNavController()
                                .navigate(
                                        R.id.action_appListFragment_to_appLimitConfigFragment,
                                        androidx.core.os.bundleOf(
                                                Constants.Key.APP_PACKAGE to it.appPackage,
                    ...
                    appHideListener = { appModel, position ->
                      ...
                      if (prefs.firstHide) {
                        ...
                        findNavController()
                                .navigate(R.id.action_appListFragment_to_settingsFragment2)
                      }
  ...
  private fun initObservers() {
    ...
    viewModel.appBlockedMessage.observe(viewLifecycleOwner) {
      it?.let { message -> requireContext().showToast(message, android.widget.Toast.LENGTH_LONG) }
    }
    if (flag == Constants.FLAG_HIDDEN_APPS) {
      ...
      override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
        ...
        when (newState) {
          ...
          RecyclerView.SCROLL_STATE_IDLE -> {
            if (!recyclerView.canScrollVertically(1)) binding.search.hideKeyboard()
